    ...
    <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST["buscar_por_cliente"])) {
                ...
                $sql = "SELECT * FROM facturas WHERE Matricula = ?";
                $consulta = $conexion->prepare($sql);
                $consulta->execute([$id_cliente]);
                $facturas = $consulta->fetchAll();
                
                echo "<div class='container'>";
                echo "<h2>Listado de Facturas del Cliente " . htmlspecialchars($id_cliente, ENT_QUOTES, 'UTF-8') . "</h2>";
                if ($facturas) {
                    ...
                    echo "<p>No se encontraron facturas para el cliente especificado.</p>";
                }
                echo "</div>";
            }
        }
